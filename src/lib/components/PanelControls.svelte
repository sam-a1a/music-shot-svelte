<script lang="ts">
    ...
    // container/sizing classes differ. Mobile rows are 44px so they are
    // comfortably tappable, which is the platform minimum for touch targets.
    const STYLE = {
        desktop: {
            ...
            checkbox: 'h-4 w-4 cursor-pointer accent-white',
            fileInput:
        ...
        mobile: {
            ...
            row: 'h-11',
            ratio: 'h-11 grid grid-cols-2 rounded-lg bg-white/10 overflow-hidden',
            theme: 'h-11 grid grid-cols-2 rounded-lg bg-white/10 overflow-hidden',
            align: 'h-11 grid grid-cols-3 rounded-lg bg-white/10 overflow-hidden',
            checkbox: 'h-5 w-5 cursor-pointer accent-white',
            fileInput:
                'block w-[70%] h-11 text-xs text-white cursor-pointer file:cursor-pointer file:h-11 file:mr-2 file:rounded-md file:border-0 file:bg-white/15 file:px-3 file:text-xs file:font-semibold file:text-white hover:file:bg-white/20',
            avatar: 'w-11 h-11 rounded-full object-cover',
            clear: 'flex-1 h-11 rounded-lg border border-white/20 bg-white/10 px-2 text-xs font-semibold text-white transition-colors hover:bg-white/20 disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed',
...
<GithubRepoButton
        ...
        className="flex {s.row} w-full items-center justify-center gap-2 rounded-lg border border-white/20 bg-white/10 px-3 text-sm font-semibold text-white transition-colors hover:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 cursor-pointer"
        iconClassName="h-4 w-4"
/>

<button
        type="button"
        class="w-full {s.row} rounded-lg border border-white/20 bg-white/10 px-3 text-sm font-semibold text-white transition-colors hover:bg-white/20 cursor-pointer"
...
<div class={s.group}>
    <div>
        ...
        <div class="flex items-center gap-2">
            <input
                    class="{s.row} w-14 cursor-pointer rounded-lg border border-white/25 bg-transparent p-1"
                    type="color"
            ...
            <button
                    type="button"
                    class="flex-1 {s.row} rounded-lg border border-white/20 bg-white/10 px-2 text-xs font-semibold text-white transition-colors hover:bg-white/20 cursor-pointer"
    ...
    {#if appSettings.showCredit}
        <div>
            ...
            <input
                    class="w-full {s.row} rounded-lg border border-white/20 bg-black/35 px-2 text-xs text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"
                    type="text"
