<script lang="ts">
    ...
    function submit() {
        albumParser.handleSubmit(appSettings.resetAccentColor)
    }

    function handleKeydown(event: KeyboardEvent) {
        if (event.key === 'Enter' && albumParser.inputUrl.trim()) {
...
<div class="relative grid h-full w-full place-items-center overflow-hidden p-4 max-md:p-3">
    ...
    >
        ...
        <div class="mb-6 pb-5 max-md:mb-5 max-md:pb-4 pt-10">
            ...
            >
                {t('app_title', appSettings.locale)}
            </h1>
            <p class="m-0 mt-2 text-[13px] leading-5 text-white/80 max-md:text-xs">
                {t('input_subtitle', appSettings.locale)}
            </p>
        </div>
        ...
        <div>
            ...
            <input
                    ...
                    name="album-url"
                    autocomplete="off"
                    placeholder={t('input_placeholder', appSettings.locale)}
                    disabled={albumParser.loading}
                    aria-invalid={!!albumParser.errorMsg || undefined}
                    aria-describedby={albumParser.errorMsg ? 'album-url-error' : undefined}
                    data-testid="album-url-input"
                    data-ai-action="input-album-url"
                    oninput={(event) => (albumParser.inputUrl = event.currentTarget.value)}
                    onkeydown={handleKeydown}
            />

            {#if albumParser.errorMsg}
                <p
                        id="album-url-error"
                        role="alert"
            ...
            <div class="relative mt-3 group">
                <button
                        ...
                        type="button"
                        disabled={albumParser.loading || !albumParser.inputUrl.trim()}
                        data-testid="parse-album-url"
                        data-ai-action="parse-album-url"
                        onclick={submit}
                >
                    {albumParser.loading
                        ? t('input_parsing', appSettings.locale)
                        : t('input_submit', appSettings.locale)}
                </button>
                {#if !albumParser.inputUrl.trim() && !albumParser.loading}
                    <div class="absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-lg bg-black/90 px-3 py-1.5 text-xs font-medium text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100 pointer-events-none shadow-lg">
                        {t('error_empty_url', appSettings.locale)}
                    </div>
                {/if}
            </div>
        </div>
      ...
      <span class="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/50">
        {t('label_supported_platforms', appSettings.locale)}
      </span>
            <span
